...
import { computeDerived } from "./steps/compute-derived";
...
export async function processWorkflow(ctx: WorkflowContext): Promise<void> {
  ...
  try {
    ...
    // Step 5: Compute derived/calculated metrics (HOMA-IR, ratios, etc.)
    const ingestedCodes = finalNormalization.normalized.map(
      (n) => n.metricCode,
    );
    await computeDerived(ctx, ingestedCodes);
