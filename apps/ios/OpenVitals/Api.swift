...
extension Api {
    ...
    struct Composed: Codable, Equatable {
        ...
        let chips: [Chip]?
        let error: String?
        /// Whether the reader ran. `false` means the words are stored unread
        /// and will be read at their own date once the reader is back.
        let read: Bool?
        /// How many times the reader has tried. Older servers omit it.
        let readAttempts: Int?

        enum CodingKeys: String, CodingKey {
            case id, reply, chips, error, read
            case readAttempts = "read_attempts"
        }
    }
