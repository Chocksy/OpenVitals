...
///
/// Signed out, there is one screen and it is the sign-in form. A session that
/// is already in the cookie jar — including one an older build got through the
/// web login — skips it, so nobody signs in twice.
import SwiftUI
...
struct OpenVitalsApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            if session.signedIn { tabs } else { SignInView() }
        }
    }
